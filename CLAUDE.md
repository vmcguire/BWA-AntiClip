# BWA-AntiClip — Read this before doing anything
...
can clone, browse, and reason about BWA-AntiClip on its own without
seeing the rest of the BWA monorepo.

All real work on BWA-AntiClip happens inside the BWA monorepo at:

```
~/Developer/bwa-fx-ecosystem/plugins/anticlip/
    dsp/           ← authoritative DSP implementation (THE source of truth)
    ui/            ← UI surface (Metal-rendered)
    plugin/        ← VST3 wrapper → builds BWA-AntiClip.vst3
    manifest.json  ← name, controls, license tier, ecosystem benefits
```

If you've been asked to "work on BWA-AntiClip," **navigate to the
...
1. **Standalone VST3** (this repo's release artifact) — `BWA-AntiClip.vst3`
...
2. **[`BWA-Architecture/docs/USER-STORY-MAP.md`](https://github.com/vmcguire/BWA-Architecture/blob/main/docs/USER-STORY-MAP.md)**
   — where BWA-AntiClip sits in the bedroom-producer journey
   (Stage 7 — MIX).
3. **[`BWA-Architecture/docs/PLAN-2026-Q2.md`](https://github.com/vmcguire/BWA-Architecture/blob/main/docs/PLAN-2026-Q2.md)**
   — current build plan + repo inventory. Find BWA-AntiClip in §9 and
...
4. **[`BWA-Architecture/docs/MARKET-PAIN-AND-ATTACKS.md`](https://github.com/vmcguire/BWA-Architecture/blob/main/docs/MARKET-PAIN-AND-ATTACKS.md)**
   — the row for BWA-AntiClip. Internalize the pain + the attack before
...
- ❌ Committing to this repo. Go to the monorepo (`plugins/anticlip/`).
...
- ❌ Adding new shared/* modules without checking if the capability folds
  ...
  Methodology Rule 3."
- ❌ Skipping the BWA-AntiClip row in MARKET-PAIN-AND-ATTACKS.md. If the
  proposed work doesn't advance the BWA Attack for this plugin, push back.

## What "done" looks like

A `BWA-AntiClip.vst3` that:
...
```bash
git subtree push --prefix=plugins/anticlip \
  git@github.com:vmcguire/BWA-AntiClip.git main
```

Then on GitHub:

1. Create a Release from `main` in this repo.
2. Attach the notarized `BWA-AntiClip.vst3` zip.
3. Reference the monorepo commit SHA in the release notes.

---

_End of LLM onboarding for BWA-AntiClip._
